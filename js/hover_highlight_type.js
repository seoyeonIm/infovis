export function addHoverEffect(svg, zoomGroup, node, link, label) {
  ...
  function handleClick(event, d) {
    if (selectedNodeId === d.id) {
      ...
      if (d.attributes?.class === "작가") {
        const artistName = d.attributes.label; // 노드의 라벨 가져오기
        console.log("작가 노드가 클릭되었습니다. 라벨:", artistName);
        showPopup(artistName); // 팝업 열기
      } else if (d.attributes?.class === "작품유형") {
        const typeKey = d.attributes.label;
        console.log("작품유형 노드가 클릭되었습니다. 라벨:", typeKey);
        updateRightBoxContent(typeKey); // 오른쪽 박스 업데이트
      }
    }
  }
  ...
  function updateRightBoxContent(typeKey) {
    ...
    const wordCloudPath = `/type_pattern_wordcloud/${typeKey}_wordcloud.png`;
    const barChartPath = `/type_artist_bar_graph/${typeKey}_bar_graph.png`;
